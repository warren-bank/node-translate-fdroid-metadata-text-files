...
translate-fdroid-metadata-text-files -i 'en' -o 'de' -o 'es' -o 'fr' -o 'zh' -o 'zh-TW' -d "$metadata_dir" -c 'icon.png' --html-entities >>"$log_file" 2>&1

metadata_dir="${DIR}/repo_02/metadata"
translate-fdroid-metadata-text-files -i 'en' -o 'de' -o 'es' -o 'fr' -o 'zh' -o 'zh-TW' -d "$metadata_dir" -c 'icon.png' --marked >>"$log_file" 2>&1
