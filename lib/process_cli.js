  ...
  for (const metadir_entry of metadir_contents) {
    if (metadir_entry.isDirectory()) {
      ...
      for (const filename of filenames) {
        ...
        let   input_filecontent = fs.readFileSync(input_filepath, {encoding: "utf8"})

        if (input_filecontent) {
          input_filecontent = apply_pre_plugins_to_input(argv_vals["--plugin"], state["filename"], input_filecontent, input_filepath)

          apply_post_plugins_to_input(argv_vals["--plugin"], state["filename"], input_filecontent, input_filepath)
          ...
          for (const output_language of argv_vals["--output-language"]) {
            ...
            if (Array.isArray(translated_strings_array) && (translated_strings_array.length === 1) && translated_strings_array[0]) {
              apply_post_plugins_to_output(argv_vals["--plugin"], state["filename"], translated_strings_array[0], output_filepath)
            }
          }
...
const apply_pre_plugins_to_input = function(plugins, input_filename, input_filecontent, input_filepath) {
  const updated_filecontent = apply_plugins(plugins, 'pre', input_filename, input_filecontent)
  ...
  return updated_filecontent
}

const apply_post_plugins_to_input = function(plugins, input_filename, input_filecontent, input_filepath) {
  const updated_filecontent = apply_plugins(plugins, 'post', input_filename, input_filecontent)
...
const apply_post_plugins_to_output = function(plugins, output_filename, output_filecontent, output_filepath) {
  output_filecontent = apply_plugins(plugins, 'post', output_filename, output_filecontent)
...
const apply_plugins = function(plugins, processing_stage, file_name, file_content) {
  if (Array.isArray(plugins) && plugins.length) {
    for (const plugin of plugins) {
      file_content = plugin(processing_stage, file_name, file_content)
    }
  }
