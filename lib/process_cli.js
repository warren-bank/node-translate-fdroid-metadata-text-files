        ...
        if (input_filecontent) {
          for (const output_language of argv_vals["--output-language"]) {
            ...
            if (Array.isArray(translated_strings_array) && (translated_strings_array.length === 1) && translated_strings_array[0]) {
              ...
              copy_files(input_dirpath, output_dirpath, argv_vals["--copy-file"])
            }
          }
        }
      }
...
const copy_files = function(input_dirpath, output_dirpath, filenames) {
  ...
  for (const filename of filenames) {
    ...
    if (fs.existsSync(input_filepath) && !fs.existsSync(output_filepath))
