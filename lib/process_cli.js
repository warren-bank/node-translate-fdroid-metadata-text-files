    ...
    if (metadir_entry.isDirectory()) {
      ...
      for (const filename of filenames) {
        ...
        if (input_filecontent) {
          apply_plugins_to_input(argv_vals["--plugin"], state["filename"], input_filecontent, input_filepath, argv_vals["--marked"])
...
const apply_plugins_to_input = function(plugins, input_filename, input_filecontent, input_filepath, as_markdown) {
  ...
  if (updated_filecontent !== input_filecontent) {
    ...
    if (as_markdown && (input_filename === 'description.txt')) {
      const backup_markdown_filename = input_filepath + '.md'

      if (!fs.existsSync(backup_markdown_filename)) {
        // backup original markdown
        fs.writeFileSync(
          backup_markdown_filename,
          input_filecontent,
          {encoding: "utf8"}
        )
      }
